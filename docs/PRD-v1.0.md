...
### 6.10 Agent interface (FR-AG) — added by DEC-24

An AI agent — Claude or any other client that speaks the **Model Context Protocol** — can set up and run a case through the same services the window uses. It is a third view of the service layer beside the window and the test suite, and it is headless for the same reason they are (NFR-M1). Implemented in `foamwb/mcp/`; launched by the client as `<app>-mcp --root <dir>`.

| ID | Requirement | Pri | Acceptance criterion |
|---|---|---|---|
| FR-AG1 | Expose case creation, inspection, byte-faithful editing, validation, geometry import, meshing dictionaries, the turbulence advisor, run planning, asynchronous runs, stopping and residuals as MCP tools. | SHOULD | Every tool is listed with a JSON schema; an agent can take a new case from creation to a finished run using tools alone. |
| FR-AG2 | Speak MCP over **stdio only**. No socket is opened, so §10.1's "no listening sockets" and NFR-R5 continue to hold. | MUST (whenever it ships) | `check_offline.py` passes with the agent interface present. |
| FR-AG3 | Confine every path an agent names to directories the user granted at launch (`--root`); symlinks are judged by where they lead. | MUST | A path outside the roots, `..` escapes and a symlink out are each refused with the permitted directories named. |
| FR-AG4 | An agent gets the window's guarantees and no others: edits through `Document.set` (FR-P7), text that does not parse is refused (E-C02), the plan is returned before a run (FR-S1), runs are recorded in the same history (FR-S7), and monitoring is installed at launch (FR-S3). | MUST | The same byte-fidelity and history assertions pass for an agent-driven edit and run. |
| FR-AG5 | Stop & Write is the default stop; SIGTERM and SIGKILL require an explicit confirmation argument — the agent's equivalent of §7.9 rule 5's dialog. Closing the pipe stops every run the server started (FR-S10). | MUST | A kill without confirmation is refused; ending the session leaves no solver process. |
| FR-AG6 | Every refusal carries its §9 code and guide anchor, so an agent's transcript is supportable exactly as a screenshot is. | MUST | Coded service failures surface their code in the tool result. |
...
| Agent interface (DEC-24) | An AI agent, or a prompt injected into one, writing or running outside the user's intent | stdio only, no socket; paths confined to user-granted roots; destructive tools annotated so the client can ask the person first; signals need explicit confirmation; every call logged (`mcp.tool.call`) so a diagnostics bundle shows what the agent did. |
...
| **DEC-24** | **An agent interface over MCP (stdio)**, hand-written on the standard library, as a Qt-free package beside `ui/` that calls the same services. Runs are asynchronous (start, poll, stop) through a Qt-free `RunJob` that shares the Run page's history code. | The MCP Python SDK; an HTTP/SSE transport; driving the GUI by automation; no agent interface | The SDK brings an HTTP stack, an async runtime and a validation library — tens of MB against NFR-P8 — to serve a transport that is one JSON object per line. An HTTP transport would open a socket, breaching §10.1 and NFR-R5. Driving the window by automation would test the pixels rather than the case. A run takes minutes to hours, far longer than any client will hold a request open, so the only honest shape is start-then-poll. It extends §2.3's "scripting" exclusion deliberately: an agent composes the existing verbs rather than recording macros. | Low — one package; nothing below it changed except the run-history helpers, which moved out of the Run page so both callers share them |
